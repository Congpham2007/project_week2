"""
smart_food_logic.py  v3.1 FINAL — BALANCED CALORIE + SIMPLE DELIVERY
=====================================================================
✅ Backend logic cho Smart Food Recommendation App
   - Fuzzy Logic Engine (FIS 1 + FIS 2)
   - OpenWeatherMap API
   - Food Database (60 món)
   - Flask REST API → HTML UI
   
📊 CALORIE TARGETS (CHUẨN - Chênh 100-200):
   - Dieter:        150-250  kcal/bữa (CẮT CALO - 200 trung bình)
   - Student:       300-400  kcal/bữa (BALANCED - 350 trung bình) → +150 từ Dieter
   - Office Worker: 400-500  kcal/bữa (BALANCED - 450 trung bình) → +100 từ Student
   - Gym:           550-650  kcal/bữa (TĂNG CƠ - 600 trung bình)  → +150 từ Office

🚚 DELIVERY TIME:
   - Công thức: delivery_time = distance_km * 15 (phút)
   - 0.3km = 4.5p ≈ 5p | 1km = 15p | 2km = 30p | 3km = 45p

🚀 Cách chạy:
    pip install flask numpy scikit-fuzzy requests python-dotenv folium
    python smart_food_logic_v3_final.py
    → http://localhost:5000
"""
...
def setup_fuzzy_engine():
    ...
    rules2 = [
        ...
        ctrl.Rule(health_condition['diet'],                           meal_type['fast']),
        ctrl.Rule(health_condition['bulking'],                        meal_type['full']),
...
def run_fis(cs1, cs2, hunger, time_avail, temp, health_val, weather_val, price_val):
    ...
    try:
        ...
        meal_type_out = sim2.output['meal_type']
        calories_out  = sim2.output['calories_level']
        
        return meal_type_out, calories_out, urgency
    except Exception as e:
        print(f"⚠️  FIS lỗi: {e}")
        return 5.0, 550.0, 5.0


# ============================================================
# 2. SIMPLE DELIVERY TIME — 1km = 15 phút
# ============================================================
def compute_delivery_simple(distance_km):
    """
    Công thức đơn giản: delivery_time = distance_km * 15
    - 0.3km = 4.5p ≈ 5p
    - 1km = 15p
    - 2km = 30p
    - 3km = 45p
    """
    delivery_minutes = max(5, round(distance_km * 15, 0))
    return float(delivery_minutes)
...
# ============================================================
# 5. CALORIE TARGETS — Profile + Meal Period (CHUẨN - Chênh 100-200)
# ============================================================
_CALORIE_TARGETS = {
    "Breakfast": {
        "Dieter":        (150, 250),      # 200 kcal
        "Student":       (300, 400),      # 350 kcal (+150 từ Dieter)
        "Office Worker": (400, 500),      # 450 kcal (+100 từ Student)
        "Gym":           (550, 650),      # 600 kcal (+150 từ Office)
    },
    "Lunch": {
        "Dieter":        (200, 300),      # 250 kcal (CẮT CALO)
        "Student":       (400, 450),      # 425 kcal (+175 từ Dieter)
        "Office Worker": (500, 600),      # 550 kcal (+125 từ Student)
        "Gym":           (650, 750),      # 700 kcal (+150 từ Office)
    },
    "Dinner": {
        "Dieter":        (150, 250),      # 200 kcal (CẮT CALO)
        "Student":       (350, 450),      # 400 kcal (+200 từ Dieter)
        "Office Worker": (450, 550),      # 500 kcal (+100 từ Student)
        "Gym":           (600, 700),      # 650 kcal (+150 từ Office)
    },
    "Late Night": {
        "Dieter":        (80, 150),       # 115 kcal (CẮT CALO)
        "Student":       (150, 200),      # 175 kcal (+60 từ Dieter)
        "Office Worker": (200, 300),      # 250 kcal (+75 từ Student)
        "Gym":           (350, 400),      # 375 kcal (+125 từ Office)
    },
}

_PROFILE_BUDGETS = {
    "Breakfast": {"Student": 40_000,  "Office Worker": 80_000,  "Gym": 100_000, "Dieter": 80_000},
    "Lunch":     {"Student": 60_000,  "Office Worker": 150_000, "Gym": 200_000, "Dieter": 120_000},
    "Dinner":    {"Student": 80_000,  "Office Worker": 300_000, "Gym": 400_000, "Dieter": 150_000},
    "Late Night":{"Student": 30_000,  "Office Worker": 80_000,  "Gym": 80_000,  "Dieter": 60_000},
}
...
_PROFILE_TYPE_MAP = {
    "Dieter":        {"fast": 1.0,  "full": 9.0},    # Thích fast
    "Student":       {"fast": 3.5,  "full": 6.5},    # Balanced
    "Office Worker": {"fast": 2.0,  "full": 8.5},    # Balanced
    "Gym":           {"fast": 0.5,  "full": 9.8},    # Thích full
}


def _get_meal_period() -> str:
    """Xác định bữa ăn theo giờ thực tế"""
    h = datetime.now().hour
    if 5 <= h < 10:
        return "Breakfast"
    elif 10 <= h < 14:
...
def _get_calorie_target(meal_period: str, profile: str) -> float:
    """
    🔴 Lấy calorie target từ _CALORIE_TARGETS (CHUẨN)
    Chênh nhau 100-200 kcal giữa các profile
    """
    if meal_period not in _CALORIE_TARGETS:
        meal_period = "Lunch"
    if profile not in _CALORIE_TARGETS[meal_period]:
        profile = "Office Worker"
    
    min_cal, max_cal = _CALORIE_TARGETS[meal_period][profile]
    target = (min_cal + max_cal) / 2.0
    print(f"📋 {profile} {meal_period} → {target:.0f} kcal")
    return target


def _cap_cal_by_meal_type(cal_target: float, meal_score: float) -> float:
    """Cap calorie nếu urgency cao (fast meal)"""
    if meal_score < 3.0:
        return min(cal_target, 400.0)
    elif meal_score < 5.0:
        return min(cal_target, 550.0)
    return cal_target


# ============================================================
# 6. SATIETY MEMORY & SCORING
# ============================================================
class SatietyMemory:
    def __init__(self):
...
def score_dishes(target_type, target_calo, meal_period,
                 memory, max_price, profile, weather_data):
    """
    Scoring + weighted random pick + simple delivery time (1km = 15p).
    🔴 LUÔN dùng _get_calorie_target() thay vì FIS calories
    """
    # 🔴 FORCE dùng calorie target từ _CALORIE_TARGETS
    actual_cal_target = _get_calorie_target(meal_period, profile)
    actual_cal_target = _cap_cal_by_meal_type(actual_cal_target, target_type)

    type_map = _PROFILE_TYPE_MAP.get(profile, {"fast": 2.0, "full": 8.5})
    ...
    for d in valid:
        ...
        pcal  = abs(actual_cal_target - d['calo']) / 500.0
    ...
    chosen = chosen.copy()
    # 🚚 SIMPLE DELIVERY: 1km = 15 phút
    chosen['delivery_time'] = compute_delivery_simple(chosen['distance_km'])
    return chosen
...
print("✅ Delivery Time: Simple (1km = 15p)", flush=True)
...
def api_recommend():
    ...
    weather_override = data.get("weather", None)
    ...
    hv = _PROFILE_HEALTH.get(profile, 5)
    meal_score, cal_score, urgency = run_fis(
        _sim1, _sim2, hunger, time_a,
        w.get("temp", 32), hv, wv, budget
    )

    meal_period = _get_meal_period()
    # 🔴 FORCE dùng _CALORIE_TARGETS
    cal_target = _get_calorie_target(meal_period, profile)
    cal_target = _cap_cal_by_meal_type(cal_target, meal_score)

    results = []
    valid = [d for d in food_db if d['price'] <= budget]
    type_map = _PROFILE_TYPE_MAP.get(profile, {"fast": 2.0, "full": 8.5})
    scored = []
    for d in valid:
        pt   = abs(meal_score - type_map[d['meal_type']]) / 10.0
        pcal = abs(cal_target - d['calo']) / 500.0
    ...
    for d, _ in scored:
        ...
        if protein_count.get(p, 0) >= 2:
            continue
        if d['style'] == "soup" and soup_count >= 2:
            continue
        protein_count[p] = protein_count.get(p, 0) + 1
        if d['style'] == "soup":
            soup_count += 1

        item = d.copy()
        # 🚚 SIMPLE DELIVERY: 1km = 15 phút
        item['delivery_time'] = compute_delivery_simple(item['distance_km'])
        item['urgency']    = round(urgency, 2)
        item['meal_score'] = round(meal_score, 2)
        item['cal_target'] = round(cal_target, 0)
        item['profile']    = profile
        results.append(item)

    return jsonify({
        "results": results, "weather": w,
        "urgency": round(urgency, 2),
        "meal_score": round(meal_score, 2),
        "cal_target": round(cal_target, 0),
...
def api_plan():
    ...
    periods = [
        {"period": "Breakfast",  "h": 3,  "t": 15,  "temp": 26},
        {"period": "Lunch",      "h": 9,  "t": 60,  "temp": 34},
        {"period": "Dinner",     "h": 7,  "t": 120, "temp": 24},
    ]
    if late:
        periods.append({"period": "Late Night", "h": 4, "t": 30, "temp": 22})
    ...
    for f in periods:
        period = f['period']
        bud = _PROFILE_BUDGETS[period].get(profile, 200_000)

        # 🔴 FORCE dùng _CALORIE_TARGETS
        cal_target = _get_calorie_target(period, profile)

        meal_score, cal_score, urgency = run_fis(
            _sim1, _sim2, f['h'], f['t'], f['temp'], hv, wv, bud
        )
        cal_target = _cap_cal_by_meal_type(cal_target, meal_score)

        chosen = score_dishes(meal_score, cal_target, period,
                              mem, bud, profile, w)
        mem.update(chosen)
...
def api_map():
    ...
    m.save(map_path)
    return jsonify({"map_url": "/food_map.html"})
...
if __name__ == "__main__":
    print("🚀 Smart Food App v3.1 FINAL — BALANCED CALORIE + SIMPLE DELIVERY")
    print("📊 Calorie: Chênh 100-200 kcal giữa các profile (Dieter → Student → Office → Gym)")
    print("🚚 Delivery: Simple formula (1km = 15 phút)")
